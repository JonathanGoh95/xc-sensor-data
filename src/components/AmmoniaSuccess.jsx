...
export default function AmmoniaSuccess({pageItems,results,handleBack,handleRefresh}){
    ...
    .map((res) => {
        ...
        const ammoniaVal = parseInt(payloadLast?.slice(25, -6), 16) / 10 || 0;
        const testRaw = parseInt(payloadLast?.slice(27, -4), 16) || '';
    ...
    const TOOLTIP_FIELDS = [
        ...
        { key: "ammoniaVal", label: "Ammonia Value (ppm)" },
        { key: "testStatus", label: "Testing Status" },
    ]

    return(
        <>
            <h1 className="font-bold italic text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-center">Ammonia Sensor Data</h1>
            ...
            <ResponsiveContainer width="105%" height="105%">
                ...
                <Line type="monotone" dataKey="ammoniaVal" name="Ammonia Value (ppm)" stroke="#3182CE" yAxisId="left" strokeWidth={2} dot={{ r: 3 }} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full md:w-4/5 justify-items-center px-4 md:px-0 py-0 md:py-4">
                {pageItems.map((res) => {
                    const sequenceNumber = parseInt(res.payload?.split(":")[res.payload.split(":").length - 1]?.slice(17, -8),16) || 0;
                    const ammoniaVal = parseInt(res.payload?.split(":")[res.payload.split(":").length - 1]?.slice(25, -6),16) / 10 || 0;
                    const testStatusRaw = parseInt(res.payload?.split(":")[res.payload.split(":").length - 1]?.slice(27, -4),16) || '';
                    let testStatus;
                    ...
                    return (
                        <div key={res.id} className="flex flex-col justify-center border-2 gap-2 p-3 md:p-2 text-center w-full rounded-md text-sm md:text-xl">
                            ...
                            <p><span className="font-bold">Ammonia Value:</span> {ammoniaVal} ppm</p>
