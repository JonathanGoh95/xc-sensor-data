export default function Search({handleSubmit,query,setQuery,queryID,setQueryID,sensorType,setSensorType}){
    return(
            <form className="flex flex-col items-center justify-center gap-4 mt-6 px-4 md:px-0" onSubmit={handleSubmit}>
                ...
                <input className="border-2 text-lg md:text-2xl rounded-lg w-auto text-center" value={queryID} type='text' placeholder='Location ID' onChange={({target})=>setQueryID(target.value)} minLength={4} maxLength={4} required></input>
